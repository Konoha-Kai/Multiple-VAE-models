...
import torch
import numpy as np
from .utils import load_data, data_process , load_h5ad
from .vae import train_vae, generate_vae, load_vae, generate_sigle_vae
...
class Bulk2Space:
    ...
    def train_vae_and_generate(self,
        ...
        # print("breed_2_list", breed_2_list)
    ...
    def train_vae_and_generate_h5ad(self,
                               input_bulk_path,
                               input_sc_h5ad_path,
                               input_st_data_path,
                               input_st_meta_path,
        ...
        input_data = load_h5ad(input_bulk_path,input_sc_h5ad_path,input_st_data_path,input_st_meta_path)
        cell_target_num = data_process(input_data, top_marker_num, ratio_num)
        single_cell, label, breed_2_list, index_2_gene, cell_number_target_num, \
        nclass, ntrain, feature_size = self.__get_model_input(input_data, cell_target_num)
        # print("breed_2_list", breed_2_list)
    ...
    def __get_multiple_model_input(self, data, cell_target_num):
        # input：data， celltype， bulk & output: label, dic, single_cell
        single_cell = data["input_sc_data"].values.T  # single cell data (600 * 6588)
        index_2_gene = (data["input_sc_data"].index).tolist()
        breed = data["input_sc_meta"]['Cell_type']
        ...
        for k, v in cell_target_num.items():
            cell_number_target_num[dic[k]] = v
        # 把single变成一个根据细胞类型为键的字典
        single_cell_dict = {}
        print('single_cell',single_cell.shape)
        index_dict = {}
        for i in range(len(breed_set)): #遍历每一种细胞类型
            # 获得每一种细胞类型的索引
            index = np.where(label == i)[0]
            #print('index',index)
            # 获得每一种细胞类型的单细胞数据
            # 注意在这里single_cell[index]是一个二维数组，需要转置
            single_cell_2 = single_cell[index].T
            # 重新设置索引，使索引从0开始
            single_cell_2 = single_cell_2[:, np.argsort(index)]
            
            single_cell_dict[breed_2_list[i]] = single_cell_2.T
            # print('single_cell_type',breed_2_list[i],'single_cell_2',single_cell_dict[breed_2_list[i]].shape)
            index_dict[breed_2_list[i]] = index

        return single_cell_dict, label, breed_2_list, index_2_gene, cell_number_target_num,index_dict, nclass, ntrain, feature_size
    ...
    def train_multiple_vae_and_generate(self,
                               input_bulk_path,
                               input_sc_data_path,
                               input_sc_meta_path,
                               input_st_data_path,
        ...
        input_data = load_data(input_bulk_path,
                               ...
                               input_st_data_path,
                               input_st_meta_path)
        # 初始化vae字典以保存每种细胞类型的vae模型
        multiple_vae_path_dict = {}
        # 初始化生成数据字典以保存每种细胞类型的生成数据
        generate_sc_data_dict = {}
        # 计算比例
        cell_target_num = data_process(input_data, top_marker_num, ratio_num)
        single_cell, label, breed_2_list, index_2_gene, cell_number_target_num,index_dict, \
        nclass, ntrain, feature_size = self.__get_multiple_model_input(input_data, cell_target_num)

        # print("label",label)

        for cell_type in input_data["input_sc_meta"]['Cell_type'].unique():
            print(f"training {cell_type} vae model")
            # 训练vae模型

            vae_net = train_vae(single_cell[cell_type],
                    label,
                    used_device,
                    batch_size,
                    feature_size=feature_size,
                    ...
                    learning_rate=learning_rate,
                    hidden_size=hidden_size)
            print(f"training {cell_type} vae model finished")
            # 因为要保存路径，所以不允许有特殊字符
            cell_type_path = cell_type.replace('/', '_')
            # 保存vae模型
            path_save = os.path.join(vae_save_dir, f"{vae_save_name}{cell_type_path}.pth")
            if not os.path.exists(vae_save_dir):
                os.makedirs(vae_save_dir)
            torch.save(vae_net.state_dict(), path_save)
            print(f"save trained vae in {path_save}.")
            multiple_vae_path_dict[cell_type] = path_save
            print(f"generating {cell_type} sc data")
            print('generating....')
            # 生成新数据
            print('cell_target_num',cell_target_num)
            #print('index_2_gene, index_dict[cell_type], cell_target_num[cell_type]', index_dict[cell_type], cell_target_num[cell_type])
            generate_sc_data = generate_sigle_vae(vae_net, -1,
                                                          single_cell[cell_type], label, breed_2_list,
                                                          index_2_gene, index_dict[cell_type], cell_target_num[cell_type],  used_device)
            path_cell_generate_csv = os.path.join(generate_save_dir, f"{generate_save_name}_{cell_type_path}_sc_data.csv")
            generate_sc_data = generate_sc_data.to_csv(path_cell_generate_csv)
            generate_sc_data_dict[cell_type] = generate_sc_data
        return multiple_vae_path_dict, generate_sc_data_dict
