...
def create_st(generate_sc_data, generate_sc_meta, spot_num, cell_num, gene_num, marker_used):
    ...
    if marker_used:
        ...
        marker_df = generate_sc_data.iloc[:, 0].dropna()
...
def create_sample(sc, st, meta, multiple):
    ...
    cfeat_p = np.array(cfeat_p_list)  # [n, d]基因个数*细胞数，d是基因数，n是细胞数
    cfeat_n = np.array(cfeat_n_list).reshape(-1, cfeat_p.shape[1])  # [n*m, d]基因个数*正样本倍数*细胞数，
