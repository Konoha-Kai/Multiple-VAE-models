...
class VAE(nn.Module):
    ...
    def forward(self, x, used_device):
        ...
        hidden = torch.randn_like(sigma) + mu * torch.exp(sigma) ** 0.5  # bvar => std
...
def train_vae(single_cell, label, used_device, batch_size, feature_size, epoch_num, learning_rate, hidden_size, ):
    ...
    weight_decay = 5e-4
    # print("single cell shape: ", single_cell.shape)
...
def prepare_single_data(cell_all_generate, label_all_generate, breed_2_list, index_2_gene):
    cell_all_generate = np.array(cell_all_generate)

    cell_all_generate_csv = pd.DataFrame(cell_all_generate)


    name = ["C_" + str(i + 1) for i in range(cell_all_generate.shape[0])]
    ...
    return  cell_all_generate_csv

def generate_sigle_vae(net, ratio, single_cell, label, breed_2_list, index_2_gene, index,cell_number_target_num=None, 
                 used_device=None):
    for p in net.parameters():  # reset requires_grad
        p.requires_grad = False  # avoid computation
    ...
    all_to_generate = cell_number_target_num
    ...
    with torch.no_grad():
        with tqdm(total=all_to_generate, desc='generating') as pbar:
            for epoch in range(epochs):
                key_list = []  # list
                generate_num = 0
                for i in range(single_cell.shape[0]):
                    if cell_number_target_num <= 0:
                        continue
                    else:
                        cell_number_target_num -= 1
                        generate_num += 1
                        key_list.append(i)

                    if cell_number_target_num == 0 or len(key_list) == 0:
                        assert cell_number_target_num == 0 
                        break
                ...
                all_to_generate -= generate_num
                pbar.update(generate_num)
                if all_to_generate == 0:
                    break

    print("generated done!","key_list",key_list)
    print("key_list",key_list)
    generate_sc_data = prepare_single_data(cell_all_generate, index, breed_2_list,
                                                      index_2_gene)
    return generate_sc_data
